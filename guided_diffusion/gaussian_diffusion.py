...
from utils import clear_color, normalize_np
# for gpu debug uses
import gc
...
class GaussianDiffusion:
    ...
    def p_sample_loop_progressive(
        ...
        for i in indices:
            ...
            if sample_method == "MCG":
                ...
                norm_grad = grad(outputs=norm, inputs=img)[0] * (1. - mask)
                out["sample"] = out["sample"] * (1. - mask) - norm_grad + \
                                self.q_sample(cond, t, th.randn(*shape, device=device)) * mask
                # del norm_grad
            elif sample_method == "vanilla":
                out["sample"] = out["sample"] * (1. - mask) + \
                                self.q_sample(cond, t, th.randn(*shape, device=device)) * mask
            elif sample_method == "repaint":
                # same as vanilla
                # re-noise
                out["sample"] = out["sample"] * (1. - mask) + \
                                self.q_sample(cond, t, th.randn(*shape, device=device)) * mask
                for j in range(repeat_steps-1):
                    betas = _extract_into_tensor(self.betas, t, out["sample"].shape)
                    # q(x_t|x_{t-1})
                    out["sample"] = th.sqrt(1 - betas) * out["sample"] \
                                    + th.sqrt(betas) * th.randn_like(out["sample"])
                    # q(x_{t-1}|x_t)
                    with th.no_grad():
                        out = self.p_sample(
                            model,
    ...
    def ddim_sample_loop_progressive(
        ...
        for i in indices:
            ...
            if task == "inpainting":
                cond = model_kwargs["ref_img"]
                if sample_method == "norm_grad":
                    # l2norm(xa - xhat)
                    # norm = th.linalg.norm((cond * mask) - (out["pred_xstart"] * mask))
                    # gradient w.r.t. z_t^b
                    norm_grad = grad(outputs=norm, inputs=img)[0] * (1. - mask)
                    scale_norm_grad = norm_grad
                    out["sample"] = out["sample"] * (1. - mask) - scale_norm_grad + \
                                    self.q_sample(cond, t, th.randn(*shape, device=device)) * mask
                elif sample_method == "vanilla":
                    out["sample"] = out["sample"] * (1. - mask) + \
                                    self.q_sample(cond, t, th.randn(*shape, device=device)) * mask
